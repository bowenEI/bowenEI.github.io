...
基于上述分析，我们不难发现一个规律：每个属七和弦都有想过渡到比它小四级的和弦的倾向。例如 VI\( _{7} \) --> II、I\( _{7} \) --> IV 等等。即使在调式中，属七和弦只有 V 级和弦一个，其他七和弦也可以强行把自己当成是属七和弦，把自己过渡到比它小四级的和弦中去。基于此，一个大胆的想法是，从 I 级和弦开始，不断地回溯比其大四级的和弦，这样会得到怎样的一个和弦进行呢？
